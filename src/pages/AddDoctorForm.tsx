...
function AddDoctorForm({ 
  ...
  const [doctorEntry, setDoctorEntry] = useState<DoctorEntryType>(editing ? doctorEditObj : initialDoctorEntry);
  ...
  function saveNewDoctor(event:FormEvent){
    ...
    axios.post("http://localhost:5173/api/doctors",
     {id:lengthDoctors + 1, ...doctorEntry})
    .then(response => {
      if(response.status===200){
        setSuccessMessage("new doctor added successfully")
 ...
 function handleEditDoctor(event:FormEvent){
  event.preventDefault()
  axios.put(`http://localhost:5173/api/doctors/${editItemId}`, {editItemId, ...doctorEntry})
  .then((response) => {
    if(response.status===200){
 ...
 console.log(editItemId)
  ...
  return (
    ...
    <Box component="form" onSubmit={editing ? handleEditDoctor : saveNewDoctor}>
        <Paper sx={{p:2, m:2, display:"flex", flexDirection:"column", alignItems:"left"}}>
        <Typography variant="h4">Doctors entry</Typography>
            <TextField  sx={{m:2}} label="Name"
             variant="outlined" 
             name = "name" value={doctorEntry.name}
            ...
            <Button type="submit" variant="contained" sx={{ ml: 5, width:100}} startIcon={<SaveIcon/>}>
                {editing ? "Update" : "Save"}
            </Button>
        </Paper>
    </Box>
